...
function CategoryRow({ category }: { category: NoteCategory }) {
        ...
        <div className="flex items-center justify-between gap-2 mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
          <span className="text-xs text-gray-600 dark:text-gray-300">
            {category.notes_count > 0
              ? `${category.notes_count} notas quedarán sin categoría. ¿Confirmar?`
              : '¿Confirmar?'}
          </span>
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <button
              type="button"
              onClick={() =>
                deleteCategory.mutate(category.id, { onSuccess: () => setConfirming(false) })
              }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="manage-categories-modal-title"
