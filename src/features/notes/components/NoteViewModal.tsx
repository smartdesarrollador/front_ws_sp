...
import { X, Copy, Check, Pin, Share2 } from 'lucide-react'
...
export function NoteViewModal({ note, open, onClose }: Props) {
  const dialogRef = useRef<HTMLDivElement>(null)
  useFocusTrap(dialogRef, open)

  const [titleCopied, copyTitle] = useCopyToClipboard()
  const [contentCopied, copyContent] = useCopyToClipboard()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      ...
      >
        ...
        <div className="p-6 space-y-4">
          {(note.category || note.tags.length > 0 || note.is_shared) && (
          ...
          <div className="flex items-center gap-4 text-xs text-gray-400 dark:text-gray-500 pt-2 border-t border-gray-100 dark:border-gray-700">
            ...
            {note.updated_at !== note.created_at && (
              <span>Actualizada: {new Date(note.updated_at).toLocaleDateString('es-ES')}</span>
            )}
          </div>
