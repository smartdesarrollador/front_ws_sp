...
function CollectionRow({ collection }: { collection: BookmarkCollection }) {
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs text-gray-400 dark:text-gray-500 flex-shrink-0">
            {collection.bookmarks_count} bookmarks
          </span>
        </div>
        {!confirming && (
          <button
        ...
        <div className="flex items-center justify-between gap-2 mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
          <span className="text-xs text-gray-600 dark:text-gray-300">
            {collection.bookmarks_count > 0
              ? `${collection.bookmarks_count} bookmarks quedarán sin colección. ¿Confirmar?`
              : '¿Confirmar?'}
          </span>
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <button
              type="button"
              onClick={() =>
                deleteCollection.mutate(collection.id, { onSuccess: () => setConfirming(false) })
              }
...
export function ManageCollectionsModal({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="manage-collections-modal-title"
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <h2
            id="manage-collections-modal-title"
